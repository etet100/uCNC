...
void gpilotSetHome(bool abs, double x, double y, double z) {
	if (abs) {
		...
		protocol_get_rt_pos(gpilotHomePos);
		gpilotHomePos[0] += x;
		gpilotHomePos[1] += y;
		gpilotHomePos[2] += z;
	}
...
//    if (cnc_get_exec_state(EXEC_HOMING))
	gpilotVirtualInputs = 0;
	{
		// xyz
		if (axis[0] < gpilotHomePos[0])
		{
			gpilotVirtualInputs |= STEP0_IO_MASK;
		}
		if (axis[1] < gpilotHomePos[1])
		{
			gpilotVirtualInputs |= STEP1_IO_MASK;
		}
		if (axis[2] > gpilotHomePos[2])
		{
			gpilotVirtualInputs |= STEP2_IO_MASK;
		}
	}
	// probe
	if (axis[2] < gpilotProbePos) {
		gpilotVirtualInputs |= STEP7_IO_MASK;
	}
